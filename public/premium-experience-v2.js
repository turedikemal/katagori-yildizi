/* Kategori Yildizi — Premium Experience v3
   Existing templates stay standard. Premium tab contains ten new animated high-end templates. */
(function(){
'use strict';
...
let wrapped=false,scheduled=false,processing=false,stageObserver=null,templateObserver=null;
...
function renderPremiumEditor(){if(!PREMIUM_IDS.has(currentTemplateId))return;const host=q('#v3TemplateEditor');if(!host)return;host.innerHTML=editorHtml(currentTemplateId);for(const [key,field] of [['Bg','bg'],['Text','text'],['Accent','accent']]){const p=q('#tpl'+key+'Picker'),h=q('#tpl'+key+'Hex');const apply=v=>{if(!/^#[0-9a-f]{6}$/i.test(v))return;palettes[currentTemplateId][field]=v;const c=palettes[currentTemplateId];window.handleInput?.(`templateColors.${currentTemplateId}`,{...c});updatePremiumCard(currentTemplateId);schedulePreview()};p.oninput=()=>{h.value=p.value;apply(p.value)};h.oninput=()=>{if(/^#[0-9a-f]{6}$/i.test(h.value)){p.value=h.value;apply(h.value)}}}}
...
function selectPremium(id){if(!PREMIUM_IDS.has(id))return;currentTemplateId=id;templateTab='premium';sessionStorage.setItem('ky-premium-template-tab','premium');applyTemplateTab();renderPremiumEditor();window.handleInput?.('templateId',id);window.handleInput?.(`templateColors.${id}`,{...palettes[id]});schedulePreview()}

function applyPreviewPremium(){if(processing||!PREMIUM_IDS.has(currentTemplateId))return;processing=true;try{const c=palettes[currentTemplateId];qa('#stageCanvas .ky-v3-badge').forEach(b=>{for(const cls of [...b.classList])if(cls.startsWith('tpl-'))b.classList.remove(cls);LEGACY_VARIANTS.forEach(x=>b.classList.remove(x));b.classList.add('tpl-'+currentTemplateId);b.style.setProperty('--badge-bg',c.bg);b.style.setProperty('--badge-text',c.text);b.style.setProperty('--badge-accent',c.accent);b.style.setProperty('color',c.text)});}finally{processing=false}}
function schedulePreview(){requestAnimationFrame(()=>{applyPreviewPremium();setTimeout(applyPreviewPremium,55)})}
function observeStage(){const stage=q('#stageCanvas');if(!stage)return;stageObserver?.disconnect();stageObserver=new MutationObserver(()=>{if(!processing&&PREMIUM_IDS.has(currentTemplateId))schedulePreview()});stageObserver.observe(stage,{childList:true,subtree:true})}
...
function wrapHandleInput(){if(wrapped||typeof window.handleInput!=='function')return;const original=window.handleInput;window.handleInput=function(path,val){if(path==='templateId'){currentTemplateId=String(val||'');if(PREMIUM_IDS.has(currentTemplateId)){templateTab='premium';sessionStorage.setItem('ky-premium-template-tab','premium')}else if(currentTemplateId){templateTab='basic'}}else if(path==='templateColors')mergePaletteMap(val);else if(/^templateColors\./.test(path)){const id=path.slice('templateColors.'.length);if(PREMIUM_IDS.has(id)&&val&&typeof val==='object')palettes[id]={...palettes[id],...val}}const out=original.apply(this,arguments);queue();if(PREMIUM_IDS.has(currentTemplateId)){setTimeout(renderPremiumEditor,0);schedulePreview()}return out};wrapped=true}
function captureStandardClick(e){const card=e.target.closest('#v3Templates .ky-template-card-v3[data-template]');if(!card)return;const id=card.dataset.template;if(PREMIUM_IDS.has(id))return;currentTemplateId=id;templateTab='basic';sessionStorage.setItem('ky-premium-template-tab','basic');setTimeout(applyTemplateTab,0)}

function queue(){if(scheduled)return;scheduled=true;requestAnimationFrame(()=>{scheduled=false;linkCss();wrapHandleInput();ensureIconTabs();ensureTemplateTabs();observeStage();if(PREMIUM_IDS.has(currentTemplateId)){renderPremiumEditor();schedulePreview()}})}
function observeTemplates(){const host=q('#v3Templates');if(!host)return;templateObserver?.disconnect();templateObserver=new MutationObserver(queue);templateObserver.observe(host,{childList:true})}
...
function start(){linkCss();document.addEventListener('click',captureStandardClick,true);queue();observeTemplates();setTimeout(()=>{queue();observeTemplates()},250);setTimeout(queue,900);setTimeout(queue,1800)}
