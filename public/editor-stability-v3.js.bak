...
function bindCategoryCard(card,index){
 ...
 qa('select[data-rank-product]',body).forEach(sel=>sel.onchange=async function(){const catId=this.dataset.rankCat,prodId=this.dataset.rankProduct,val=this.value;if(!catId||!prodId)return;const manualRank=val?Number(val):null;const hideInput=q(`input[data-hide-product="${CSS.escape(String(prodId))}"][data-hide-cat="${CSS.escape(String(catId))}"]`,body);const currentHidden=hideInput?hideInput.checked:undefined;try{const r=await fetch(`/api/admin/categories/override?shop=${encodeURIComponent(shop)}`,{method:'POST',body:JSON.stringify({categoryId:catId,productId:prodId,manualRank,...(currentHidden!==undefined?{hidden:currentHidden}:{})}),headers:{'Content-Type':'application/json'}});if(!r.ok)throw new Error((await r.json()).message||'Sıralama güncellenemedi');q('#v3Sync')?.click();}catch(e){console.error('[Rank select]',e);this.value='';}});
