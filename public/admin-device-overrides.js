...
function legacySnapshot(raw){
 const c=raw||{};
 return {
   templateId:c.templateId||'navy-pill',
   templateColors:clone(c.templateColors||{}),
   texts:clone(c.texts||{}),
   styling:clone(c.styling||{}),
   icon:clone(c.icon||{}),
   placements:clone(c.placements||{}),
   animation:clone(c.animation||{})
 };
}
...
function applyProfile(device){
 if(!deviceSettings[device])return;
 ...
 setTimeout(()=>{syncControlsFromProfile();syncDeviceTabs();enforceTemplateCapabilities();},20);
}
function walk(obj,fn,prefix=''){
 Object.entries(obj||{}).forEach(([k,v])=>{const path=prefix?`${prefix}.${k}`:k;if(v&&typeof v==='object'&&!Array.isArray(v))walk(v,fn,path);else fn(path,v);});
}
function syncControlsFromProfile(){
 const p=deviceSettings[activeDevice];if(!p)return;
 internal=true;
 try{
...
function injectDeviceTabs(){
 VISUAL_PANELS.forEach(id=>{
   ...
   const head=q('.ky-subpanel-header',panel);
   const tabs=document.createElement('div');tabs.className='ky-device-editor-tabs';tabs.innerHTML='<button type="button" data-edit-device="desktop">Masaüstü</button><button type="button" data-edit-device="mobile">Mobil</button>';
   (head?.nextSibling?panel.insertBefore(tabs,head.nextSibling):panel.prepend(tabs));
 });
 syncDeviceTabs();
}
function reorderMenu(){
 ...
 if(icons){q('.ky-menu-title',icons).textContent='Rozetler';const sub=q('.ky-menu-subtitle',icons);if(sub)sub.textContent='Rozet ikonları, premium seçenekler ve boyut';}
 if(q('#panelIcons .ky-subpanel-title'))q('#panelIcons .ky-subpanel-title').textContent='Rozetler';
 if(tplPanel&&icons&&iconsPanel){tplPanel.after(icons);icons.after(iconsPanel);}
 if(iconsPanel&&borders&&bordersPanel){iconsPanel.after(borders);borders.after(bordersPanel);}
 const resp=q('.ky-menu-card[data-target="panelResponsive"] .ky-menu-title');if(resp)resp.textContent='Cihaz Görünürlüğü';
 const respHead=q('#panelResponsive .ky-subpanel-title');if(respHead)respHead.textContent='Cihaz Görünürlüğü';
}
...
function addAnimationOptions(){
 ...
 const hover=q('#v3Hover');if(hover&&!q('option[value="slide-left"]',hover)){
   [['slide-left','Sola Kaydır'],['slide-right','Sağa Kaydır']].forEach(([v,t])=>{const o=document.createElement('option');o.value=v;o.textContent=t;hover.appendChild(o);});
 }
}
function enforceTemplateCapabilities(){
 const id=activeTemplate();
 const gradientOk=SIMPLE_GRADIENT_TEMPLATES.has(id),radiusOk=RADIUS_TEMPLATES.has(id),borderOk=BORDER_TEMPLATES.has(id);
 ...
 if(grad){grad.disabled=!gradientOk;const row=grad.closest('.ky-toggle-wrap');if(row)row.classList.toggle('ky-control-disabled',!gradientOk);}
 if(gradFields)gradFields.classList.toggle('ky-control-disabled',!gradientOk);
 ...
 if(borderColor)borderColor.closest('.ky-field')?.classList.toggle('ky-control-disabled',!borderOk);
 let note=q('#kyTemplateCapabilityNote');
 const host=q('#panelColors .ky-group');
...
function enhanceCategories(){
 qa('#v3Categories .ky-category-card').forEach((card,i)=>{
   if(card.dataset.kyCollapsible)return;card.dataset.kyCollapsible='1';
   const head=q('.ky-category-head',card);if(!head)return;
   const rows=qa('.ky-rank-row',card);const body=document.createElement('div');body.className='ky-category-body';rows.forEach(r=>body.appendChild(r));card.appendChild(body);
   const arrow=document.createElement('span');arrow.className='ky-category-arrow';arrow.textContent='⌄';head.appendChild(arrow);
   card.classList.toggle('open',i===0);
...
function removePdpOuterFrame(){qa('.ky-focus-pdp-badge,.ky-live-pdp-badge').forEach(el=>{el.style.setProperty('border','0','important');el.style.setProperty('box-shadow','none','important');el.style.setProperty('background','transparent','important');el.style.setProperty('padding','0','important');});}
...
function bind(){
 ...
 document.addEventListener('input',e=>{setTimeout(()=>{mirrorControl(e.target);captureTemplateColor(e.target);},0);},true);
 document.addEventListener('change',e=>{setTimeout(()=>{mirrorControl(e.target);captureTemplateColor(e.target);},0);},true);
}

function refreshUi(){reorderMenu();injectDeviceTabs();simplifyResponsive();addAnimationOptions();enforceTemplateCapabilities();enhanceCategories();removePdpOuterFrame();syncDeviceTabs();}
const observer=new MutationObserver(()=>{if(!ready)return;requestAnimationFrame(refreshUi);});
...
async function start(){
 ...
 applyProfile(activeDevice);
 refreshUi();
 observer.observe(document.body,{subtree:true,childList:true});
 setTimeout(refreshUi,500);setTimeout(refreshUi,1400);
}
